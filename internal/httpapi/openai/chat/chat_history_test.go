...
func TestChatCompletionsCurrentInputFilePersistsNeutralPrompt(t *testing.T) {
	...
	if ds.uploadCalls[0].Filename != "CONV_LOG.txt" {
		t.Fatalf("expected CONV_LOG.txt upload, got %q", ds.uploadCalls[0].Filename)
	}
	...
	if !strings.Contains(full.Messages[0].Content, "Continue from the latest state in the attached CONV_LOG.txt context.") {
