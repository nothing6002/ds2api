...
func TestStartCompletionAppliesCurrentInputFileGlobally(t *testing.T) {
	...
	if got := ds.uploads[0].Filename; got != "CONV_LOG.txt" {
		t.Fatalf("upload filename=%q want CONV_LOG.txt", got)
	}
	...
	if !start.Request.CurrentInputFileApplied || !strings.Contains(start.Request.PromptTokenText, "# CONV_LOG.txt") {
